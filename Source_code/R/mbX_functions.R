...
ezclean <- function(microbiome_data, metadata, level = "d") {
  ...
  # ---- Final Return Block ----
  final_names <- list(
    "d__" = paste0("mbX_cleaned_domains_or_kingdom_", microbiome_base, ".xlsx"),
    "p__" = paste0("mbX_cleaned_phylum_",                     microbiome_base, ".xlsx"),
...
ezviz <- function(microbiome_data, metadata, level, selected_metadata,
  ...
  vizDataNames <- list(
    "d__" = paste0("mbX_vizualization_data_domains_",  "_", microbiome_base,  "_", selected_metadata , ".xlsx"),
    "p__" = paste0("mbX_vizualization_data_phyla_", microbiome_base,  "_", selected_metadata , ".xlsx"),
    "c__" = paste0("mbX_vizualization_data_classes_", microbiome_base,  "_", selected_metadata , ".xlsx"),
    "o__" = paste0("mbX_vizualization_data_orders_", microbiome_base,  "_", selected_metadata , ".xlsx"),
    "f__" = paste0("mbX_vizualization_data_families_",microbiome_base,  "_", selected_metadata , ".xlsx"),
    "g__" = paste0("mbX_vizualization_data_genera_", microbiome_base,  "_", selected_metadata , ".xlsx"),
    "s__" = paste0("mbX_vizualization_data_species_", microbiome_base,  "_", selected_metadata , ".xlsx")
  )
  ...
  final_names_viz <- list(
    "d__" = paste0("mbX_viz_domains_or_kingdom_", microbiome_base, "_", selected_metadata),
    "p__" = paste0("mbX_viz_phylum_",                     microbiome_base, "_", selected_metadata, ".pdf"),
    "c__" = paste0("mbX_viz_classes_",                    microbiome_base, "_", selected_metadata, ".pdf"),
    "o__" = paste0("mbX_viz_orders_",                     microbiome_base, "_", selected_metadata, ".pdf"),
    "f__" = paste0("mbX_viz_families_",                   microbiome_base, "_", selected_metadata, ".pdf"),
    "g__" = paste0("mbX_viz_genera_",                     microbiome_base, "_", selected_metadata, ".pdf"),
    "s__" = paste0("mbX_viz_species_",                    microbiome_base, "_", selected_metadata, ".pdf")
  )
  ...
  final_names_viz_dir <- list(
    "d__" = paste0("mbX_viz_domains_or_kingdom_", microbiome_base),
    "p__" = paste0("mbX_viz_phylum_",                     microbiome_base),
...
ezstat <- function(microbiome_data, metadata, level, selected_metadata) {
  ...
  #
  microbiome_base <- tools::file_path_sans_ext(basename(microbiome_data))
  dir_name        <- paste0("ezstat_outputs_", canonical_level, "_", microbiome_base)
  ...
  #
  #  8) Write "ezstat_KW_tests_<canonical_level>_<selected_metadata>.xlsx"
  #
  out_kw <- file.path(
    dir_name,
    paste0("ezstat_KW_tests_", canonical_level, "_", selected_metadata, ".xlsx")
  )
  openxlsx::write.xlsx(results_kw, file = out_kw, rowNames = FALSE)
  ...
  out_pw <- file.path(
    dir_name,
    paste0("ezstat_pairwise_", canonical_level, "_", selected_metadata, ".xlsx")
  )
  ...
  for (tax in sig_taxa) {
    ...
    # 10.2) Run FSA::dunnTest() with safe fallback
    dunn_out <- tryCatch(
      FSA::dunnTest(x = temp_df$Value, g = temp_df$Group, method = "bh"),
      error = function(e) NULL
    )
    
    if (is.null(dunn_out) || nrow(dunn_out$res) == 0) {
      # if it failed, everyone gets "a"
      groups_present <- sort(unique(temp_df$Group))
      cld_df <- data.frame(
        Group = groups_present,
        CLD   = rep("a", length(groups_present)),
        stringsAsFactors = FALSE
      )
    } else {
      dunn_res2 <- dunn_out$res
      ...
      if (n_groups == 2) {
        # two_group special case
        padj    <- dunn_res2$P.adj[1]
        ...
        if (length(pval_vec) == needed) {
          ...
          # fallback if some comparisons missing
          cld_df <- data.frame(
            Group = groups_present,
            CLD   = rep("a", n_groups),
            stringsAsFactors = FALSE
  ...
  out_cld <- file.path(
    dir_name,
    paste0("ezstat_CLD_Summary_", canonical_level, "_", selected_metadata, ".xlsx")
  )
